...
class VGGBlock(Layer):
    def __init__(self, filters, num_convs=2, **kwargs):
        super().__init__(**kwargs)
        self.convs = []
        for _ in range(num_convs):
            self.convs.append(Conv2D(filters, 3, padding='same', use_bias=False))
...
def get_hybrid_modelVGGG(input_shape):
    ...
    model = Model(inputs, outputs, name="Hybrid_Model_VGG")
    return model

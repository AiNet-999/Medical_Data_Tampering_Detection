...
def save_splits(
    ...
    print(f"{split_name} FM count : {fm_count}")
    print(f"{split_name} FB count : {fb_count}")


    random.shuffle(real_pool)

    total_fake = fm_count + fb_count

    if len(real_pool) < total_fake:
        print(
            f"WARNING: only {len(real_pool)} real slices "
            f"available for {split_name}"
        )

    selected_real = real_pool[:total_fake]
    ...
    print(f"{split_name} Real count : {real_used}")

    print(f"\n{split_name} SUMMARY")
    print(f"FM   : {fm_count}")
    print(f"FB   : {fb_count}")
    print(f"Real : {real_used}")
